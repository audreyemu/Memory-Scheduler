...
int round_robin = 1; // set default to thread id of 1
...
void schedule(int channel)
{
	...
	if(!drain_writes[channel])
	{
		bool count_reached = false; // to check if we even reach a command with this thread id
		int temp = 1; // default thread id is 1

		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			if(rd_ptr->thread_id == round_robin) { // if this command has the correct thread id
				count_reached = true;
				if(rd_ptr->command_issuable && dram_state[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank].state == ROW_ACTIVE) // and its row is active
				{
					...
					issue_request_command(rd_ptr); // issue command and increment round robin value
					round_robin++;
					return;
				}
			}
		...
		if(!count_reached) {
			temp = round_robin;
			round_robin = 1; // if we didn't even reach something with that thread id, our round robin number might be too high
		}

		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			if(rd_ptr->command_issuable && dram_state[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank].state == ROW_ACTIVE) // if the row is active
			{
				if (rd_ptr->next_command == COL_READ_CMD) {
				  recent_colacc[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = 1;
				}
				...
				issue_request_command(rd_ptr); // issue the command
				return;
			}
		}

		round_robin = temp; // if we get here, we try round robin again (without checking for active row)

		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			if(rd_ptr->thread_id == round_robin) { // if correct thread id and command is issuable, issue the command and increment round robin value
				count_reached = true;
		...
		if(!count_reached) {
			round_robin = 1; // set to default if we never reach it
		}

		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			if(rd_ptr->command_issuable) // if any command is issueable, issue first one found
			{
				if (rd_ptr->next_command == COL_READ_CMD) {
				  recent_colacc[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = 1;
				}
